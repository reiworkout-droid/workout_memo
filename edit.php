...
$sql = 'SELECT * FROM workout_memo WHERE id=:id';
            ...
            <div class="textArea">
                    ...
                    <select name="categories[]" class="categorySelect" required>
                        ...
                        <?php foreach ($categories as $cat): ?>
                            <?php
                                ...
                                $selected = ($key === $result['categories']) ? 'selected' : '';
                            ?>
                ...
                <div class="menu">
                    <select name="menu[]" class="menuSelect" required>
                        <option value="<?= htmlspecialchars($result['menu']) ?>" selected>
                            <?= htmlspecialchars($result['menu']) ?>
                        </option>
                    </select>
                </div>
                <div class="mainText">
                    <div class="numberText">
                        <div class="weight">
                            <input type="number" name="weight[]" value="<?= $result['weight'] ?>"><span>kg</span>
                        </div>
                        <div class="reps">
                            <input type="number" name="reps[]" value="<?= $result['reps'] ?>"><span>回</span>
                        </div>
                        <div class="max">
                            1RM: 
                    ...
                    <div class="memo">
                        メモ: <input type="text" name="memo[]" value="<?= $result['memo'] ?>">
                    </div>
                </div>
            </div>


            <input type="hidden" name="date" value="<?= htmlspecialchars($result['date']) ?>">
